...
#Jaccard Similarity Coefficient, Clustering method: Mixture of normal distributions with nine clusters
...
for (k in 1:ncluster){
	...
	for (j in 1:M){
		...
		cl=Mclust(newdata[c(9:12)],model="VVV",G=ncluster)
		jaccard=numeric(ncluster)
		for(i in 1:ncluster){
			nk=data3[which(clusternew==k),]
			Cstar=intersect(nk,newdata)
			delta=newdata[which(cl$classification==i),]
